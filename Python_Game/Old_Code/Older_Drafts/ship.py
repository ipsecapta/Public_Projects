...
class Ship(pygame.sprite.Sprite): #create a Ship class, that is a sub-class of the pygame Sprite class.
    def __init__(self, settings, screen, player_id, player_level=1, score = 0):
        super().__init__()
        ...
        self.image = pygame.image.load(image_path).convert_alpha() #use the convert_alpha() method to load the image such that its transparent background is maintained (as a png)
        self.image = pygame.transform.smoothscale(self.image, (60, 40)) #set each image scale to 60x40 pixels.
    ...
    def update(self): # a function for ship movement
        ...
        if self.moving_left and self.rect.left > 0: # if the left side of the ship sprite's rectangle is not past x=0 (the left edge of screen), -->
            self.x -= self.settings.ship_speed #--> then increment ship position leftward by one unit of the ship_speed defined in the settings file.
        if self.moving_right and self.rect.right < self.settings.screen_width: #if the right side of the ship's sprite's rectangle is not past the maximum screen width (screen right edge)-->
            self.x += self.settings.ship_speed #-->then increment ship position rightward by one unit of the ship_speed etc.
