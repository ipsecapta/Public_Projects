    ...
    def _update_wave_spawning(self):
                    ...
                    if self.level2_remaining > 0 and now >= self.next_level2_spawn_time:
                         ...
                         self.next_level2_spawn_time = (now + random.randint
                                                        (self.settings.alien2_min_spawn_interval, 
                                                        self.settings.alien2_max_spawn_interval)
                                                            )
                    ...
                    if self.level3_remaining > 0 and now>=self.next_level3_spawn_time:
                         ...
                         self.next_level3_spawn_time = (now + random.randint
                                                        (self.settings.alien3_min_spawn_interval, 
                                                         self.settings.alien3_max_spawn_interval)
                                                            )
                    ...
                    if self.level4_remaining > 0 and now>=self.next_level4_spawn_time:
                         ...
                         self.next_level4_spawn_time = (now + random.randint
                                                        (self.settings.alien4_min_spawn_interval, 
                                                         self.settings.alien4_max_spawn_interval)
                                                            )
                         

                    if self.destroyers_remaining > 0 and now>=self.next_destroyer_spawn_time:
                         self._spawn_single_alien(level=5) #destroyer
                         self.destroyers_remaining -= 1
                         self.next_destroyer_spawn_time = (now + random.randint
                                                        (self.settings.destroyer_min_spawn_interval, 
                                                         self.settings.destroyer_max_spawn_interval)
                                                            )                                            
        ...
        for bullet, aliens_hit in alien_is_hit.items():
                ...
                for alien in aliens_hit:
                    ...
                    if destroyed:
                        ...
                        except: #since "boss" level is a string, a try/exceot works well here to score the boss, without having to change the overall scoring equation above.
