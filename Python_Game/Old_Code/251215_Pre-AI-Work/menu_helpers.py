#Menu_helpers.pygame
#file for the functions that help with the opening menu.
...
def gamesetup(self, ShipClass): #note that the ship class is passed to this function from the main file, so it can create ship objects for the main file. 
        ...
        while self.current_wave_num not in (0,1,2,3,4):
                while True:
                  self.current_wave_num = input("Select a wave number to start on - 1 through 5:   ")
                  try:                        
                        if int(self.current_wave_num) in (1,2,3,4,5): #check the selected wave number
                            self.current_wave_num = int(self.current_wave_num)
        ...
        while readyornot.lower() != "ready":
                 ...
                 """  failcounter += 1
                 while 3 < failcounter < 6 and readyornot.lower() != "ready":
                      readyornot = input("CADET! This is no time for tomfoolery! When ready, type READY to get out there and SAVE THE EARTH!")
                      failcounter += 1
                 while failcounter """
        ...
        while pregame_pause:
            ...
            for event in pygame.event.get(): #listen for any keystrokes
                ...
                if event.type == pygame.KEYDOWN: #if any key is pressed, start the game:
                    ...
                    pregame_pause=False
            self.clock.tick(30) # AI recommended this, as it makes the loop wait 30 milliseconds before running again, keeping it from 'running too hot'
    #Once setup is complete, call the main window back, and load everything onto it.
