...
#GAME UPDATE FUNCTION:       
    ...
    #First, check for any between-wave pauses:
        ...
        if self.game_state == "between_waves":
            ...
            return
    #allow alien bullets, as well as player ships and bullets, to still move if victory:
        if self.game_state == "victory":
            self.players.update()
            self.player_bullets.update()
            self.alien_bullets.update()
            return
    #allow only alien bullets and ships to move if defeat:
    ...
    def _update_wave_spawning(self):
                    ...
                    #In future: add cruisers, laser tankers, and bombarders to this list.                                            
    ...
    def _spawn_single_alien(self, level: int):
                    ...
                    elif new_alien.level == 6: #cruiser - for later in development, when I have these game-ready, I don't want these too close to the edge either b/c of their firing pattern.
    ...
    def _alien_firing_logic(self):
        ...
        for alien in self.aliens:
             if alien.ready_to_fire(now): #is alien fire timer set to current time?
                  self.fire_alien_bullet(alien, alien.level) #then fire a bullet
        ...
        if player_is_hit:
             for bullet, ships_hit in player_is_hit.items(): #note that in this for loop, the order of the two iterating variables has to match the -->
                  ...
                  for ship in ships_hit:
                    ...
                    print (f"PLAYER ID {ship.player_id}, your ship was just hit and you now have {ship.player_health} health left!")
...
######WAVE DYNAMICS FUNCTIONS:    
    ...
    def _draw_screen(self):
        ...
        #(eventually: add draw shields, given their health, etc)"
