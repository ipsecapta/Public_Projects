...
#       All images whose paths in the /img directory are referenced (e.g. see lines 21-28 below); plus variables in base_settings holding-->
#       --> other image paths, accessed through the settings class handed down by the main file when it creates aliens.
#
# Author: Anthony Visintainer
# Date: 8 December 2025
#
#This module models alien behavior, damage, and visuals. 
...
class Alien(pygame.sprite.Sprite): #Create a class for aliens level 1-3
    def __init__(self, settings, screen, level=1):
        ...
        self.resizer = self.settings.multiplayer_resizer #call the multiplayer_resizer factor from settings, and give short in-scope variable name for convenience
        ...
        if level == 1:
            ...
            self.spritesize = (round(50*self.resizer), round(50*self.resizer))  #set the size of the sprite as a tuple, for multiple uses later
        elif level == 2: 
            img_path = ALIEN2_PATH
            self.speed = settings.alien2_speed
            self.spritesize = (round(40*self.resizer), round(60*self.resizer))
        elif level == 3:
            img_path = ALIEN3_PATH
            self.speed = settings.alien3_speed
            self.spritesize = (round(40*self.resizer), round(69*self.resizer))
        elif level == 4:
             img_path = ALIEN4_PATH
             self.speed = settings.alien4_speed
             self.spritesize = (round(40*self.resizer), round(90*self.resizer))
        elif level == 5: #destroyers are level 5
             img_path = DESTROYER_PATH
             self.speed = settings.destroyer_speed
             self.spritesize = (round(100*self.resizer), round(143*self.resizer))
